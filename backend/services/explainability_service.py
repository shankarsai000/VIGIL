...
logger = logging.getLogger("vigil.services.explainability")
...
class ExplainabilityService:
    ...
    def __init__(self, api_key: str = None):
        ...
        self._mock_mode = (not self.api_key) or (not ANTHROPIC_AVAILABLE)
        
        if not self._mock_mode:
            logger.info("Initializing Anthropic explainer client for explainability_service...")
            self.client = anthropic.AsyncAnthropic(api_key=self.api_key)
        else:
            logger.info("Anthropic explainer starting in MOCK / RULE-BASED mode.")

    def _generate_cache_key(self, incident: Any) -> str:
    ...
    async def generate_explanation(self, incident: Any) -> str:
    ...
    def generate_rule_based(self, incident: Any) -> str:
        ...
        decision = "approved" if incident.policy_decision.value == "APPROVED" else "escalated"
        if incident.policy_decision.value == "DENIED":
            decision = "overruled"

        # Sentence 1: The Attack
        if threat.value == "PROMPT_INJECTION":
            s1 = f"A user attempted to hijack {agent_name} by injecting a prompt instructing it to bypass safety boundaries and export data."
        elif threat.value == "PRIVILEGE_ESCALATION":
            s1 = f"The {agent_name} agent attempted to execute an unauthorized database operation and call the '{target}' tool."
        elif threat.value == "UNAUTHORIZED_DELEGATION":
            s1 = f"The {agent_name} agent attempted to bypass delegation rules by calling the restricted agent '{target}'."
        elif threat.value == "DATA_EXFILTRATION":
            s1 = f"An attacker tried to exfiltrate massive blocks of sensitive records using {agent_name}'s channels."
        elif threat.value == "RATE_ANOMALY":
        ...
        elif threat.value == "VOLUME_ANOMALY":
        ...
        time_elapsed = int((incident.timestamp % 1) * 100) + 12
